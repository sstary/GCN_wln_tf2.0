...
parser.add_option("-m", "--save_dir", dest="save_path", default="model-300-3-direct_mxp")
...
_input_atom = tf.compat.v1.placeholder(tf.float32, [batch_size, None, adim])
_input_bond = tf.compat.v1.placeholder(tf.float32, [batch_size, None, bdim])
_atom_graph = tf.compat.v1.placeholder(tf.int32, [batch_size, None, max_nb, 2])
_bond_graph = tf.compat.v1.placeholder(tf.int32, [batch_size, None, max_nb, 2])
_num_nbs = tf.compat.v1.placeholder(tf.int32, [batch_size, None])
_node_mask = tf.compat.v1.placeholder(tf.float32, [batch_size, None])
_src_holder = [_input_atom, _input_bond, _atom_graph, _bond_graph, _num_nbs, _node_mask]
_label = tf.compat.v1.placeholder(tf.int32, [batch_size, None])
_binary = tf.compat.v1.placeholder(tf.float32, [batch_size, None, None, binary_fdim])

# Queueing system allows CPU to prepare a buffer of <100 batches
q = tf.queue.FIFOQueue(100, [tf.float32, tf.float32, tf.int32, tf.int32, tf.int32, tf.float32, tf.int32, tf.float32])
...
loss = tf.nn.sigmoid_cross_entropy_with_logits(labels=flat_score, logits=tf.cast(flat_label, tf.float32))
...
def read_data(path, coord):
    ...
    bucket_size = [10,15,20,25,30,35,40,45,50,55]
